...
function makeEvent(
  overrides: Partial<PipelineStreamEvent> = {},
): PipelineStreamEvent {
  return {
    pipelineRunId: 42,
    step: 'RSS_COLLECT',
...
describe('useActivePipeline', () => {
  ...
  it('initialises with default pipeline steps from getActivePipeline', async () => {
    ...
    expect(
      result.current.data?.steps.every((s) => s.status === 'WAITING'),
    ).toBe(true);
  ...
  it('transitions the active step to RUNNING', async () => {
    ...
    act(() =>
      capturedOnEvent!(makeEvent({ step: 'RSS_COLLECT', status: 'RUNNING' })),
    );
  ...
  it('marks all preceding steps as SUCCESS when a later step becomes active', async () => {
    ...
    act(() =>
      capturedOnEvent!(makeEvent({ step: 'ARTICLE_SAVE', status: 'RUNNING' })),
    );
  ...
  it('ignores events where pipelineRunId is null', async () => {
    ...
    act(() =>
      capturedOnEvent!(makeEvent({ pipelineRunId: null as unknown as number })),
    );
